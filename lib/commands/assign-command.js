...
var Promise, formatChanges, newRequests, newSpace, onIssueCommented, onIssueCreated, onIssueUpdated, sendToChat;
...
onIssueCreated = function(robot, space, project, w) {
  ...
  message = "新しい課題「" + issueKey + " " + summary + "」を " + username + " が追加したみたい。\n" + description + "\n" + issueUrl;
...
onIssueUpdated = function(robot, space, project, w, requests) {
  ...
  message = "課題「" + issueKey + " " + summary + "」を " + username + " が更新したみたい。\n" + comment + "\n```\n" + changes + "\n```\n" + issueUrl;
  m = {
    room: room,
    message: message
  };
...
onIssueCommented = function(robot, space, project, w) {
  ...
  comment = w.content.comment.content;
  room = project.getRoom();
  message = "課題「" + issueKey + " " + summary + "」に " + username + " がコメントしたみたい。\n" + comment + "\n" + issueUrl + "#comment-" + commentId;
  return sendToChat(robot, {
    room: room,
    message: message
  });
...
module.exports = function(arg) {
  var config, requests, robot, space;
