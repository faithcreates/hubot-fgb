...
Space = (function() {
  ...
  Space.prototype.returnIssue = function(issueKey) {
    return this.backlog.getIssueComments(issueKey).then((function(_this) {
      return function(comments) {
        var assigner, projectKey;
        assigner = _this.getPreviousAssigner(comments);
        projectKey = issueKey.replace(/-\d+$/, '');
        return _this.backlog.getProjectUsers(projectKey).then(function(users) {
          var filtered, user;
          filtered = users.filter(function(i) {
            return i.name === assigner;
          });
          user = filtered[0];
          if (user == null) {
            return;
